...
$(document).ready(
	function(){
		...
		$('.navigation-main').each(function(){
				...
				var w = $(this).find('li:last a').width();
				$(this).find('li:last a').width(w+(container_w-content_w2));
			});
		...
		$('.tab-nav li').click(function(){
			...
			if (jQuery.isFunction(init_search_partner_map)) {
				//if ($.browser.msie!==false && $.browser.version==8) alert("ok");
				//search_partner_map_timer = setTimeout('init_search_partner_map',100);
				init_search_partner_map();
			}
		});
		...
		$('.stage-gallery').each(function(){
		
			var has_nav_stage = false;
			if ($(this).find('.nav-stage').length) has_nav_stage = true;
		
			var l = $(this).find('.stage-gallery-tn img').length;
			$(this).data('total',l);
			
			loadStageGalleryImage = function(index) {
				var url = $('.stage-gallery-tn a:eq('+index+')').attr('href');
				var caption = $('.stage-gallery-tn a:eq('+index+') img').attr('alt');
				var previous = index-1;
				if (previous==-1) previous = $('.stage-gallery').data('total')-1;
				var next = index+1;
				if (next>=$('.stage-gallery').data('total')) next = 0;
				$('.stage-gallery .large .img-container').addClass('img-container-old').removeClass('img-container');
				$('.stage-gallery .large .img-container-old').each(function(){
					$(this).animate({opacity:0},300,function(){
						$(this).remove();
					});
				});
				$('.stage-gallery .large').append('<div class="img-container" style="visibility:hidden"><img src="'+url+'" /></div>');
				if (caption!='') $('.stage-gallery .large .img-container').append('<div class="caption">'+caption+'</div>');
				$('.stage-gallery .large .img-container img').load(function(){
					//$('.stage-gallery .large .img-container-old').remove();
					var w = $(this).width();
					if (w > 800) $(this).width(800);
					var h = $(this).height();
					if (h > 600) $(this).height(600);
					w = $(this).width();
					h = $(this).height()+$(this).parents('.img-container').find('.caption').height()+10;
					$(this).parent().width(w);
					var l = ((960-w)/2)+'px';
					//$(this).parent().css('left','1000px');
					$(this).parent().css({left:l});
					$(this).parents('.body').clearQueue().animate({height:h},200);
					$(this).parent().css({visibility: 'visible', opacity:0}).animate({left:l,opacity:1},500);
					$(this).parents('.large').css('height',h);
					$(this).parents('.large').find('.icon').show();
					$(this).unbind('click').click(function(){
						loadStageGalleryImage(next);
					});
					$(this).parents('.large').find('.icon-next').unbind('click').click(function(){
						loadStageGalleryImage(next);
					});
					$(this).parents('.large').find('.icon-prev').unbind('click').click(function(){
						loadStageGalleryImage(previous);
					});
				});
			}		
			var complete_w = 0;	
			$(this).find('.stage-gallery-tn img').each(function(){
				w = $(this).width();
				m = parseInt($(this).parent().css('margin-right').replace(/px/,''));
				complete_w += w+m;
				if ($(this).parents('.nav-stage').length==0) {
					$(this).parent().click(function(){
					
						var i = $(this).attr('href');
						var index = $(this).index();
						$('.stage-gallery').data('current',index);
						$(this).parents('.stage-gallery-tn').hide();
						$(this).parents('.body').find('>.icon').hide();
						$(this).parents('.body').append('<div class="large"><div class="icon icon-prev hidden" /><div class="icon icon-next hidden" /><div class="icon-close" /></div>');
						$(this).parents('.body').find('.icon-close').click(function(){
							$(this).parents('.body').find('.stage-gallery-tn,>.icon').fadeIn(400);
							$(this).parents('.body').animate({height:140},200);
							$(this).parents('.body').find('.large').remove();
						});
						loadStageGalleryImage(index);
						return false;
					
					});
				}
			}); 
			if ($.browser.msie && $.browser.version<=7) { complete_w += 30; }
			var min_x = $(this).find('.body, .stage-gallery-tn-container').width()-complete_w+m+18;
			if (has_nav_stage===true) min_x -= 27;
			$(this).find('.stage-gallery-tn').width(complete_w).data('min_x',min_x);
			if (l>4) {
				$(this).find('.body,.stage-gallery-tn-container').append('<div class="icon icon-next" /><div class="icon icon-prev">');
			}
			$(this).find('.body > .icon, .stage-gallery-tn-container > .icon').hover(function(){
				var dir = 'left';
				if ($(this).hasClass('icon-prev')) dir = 'right';
				window.stageGalleryInterval = setInterval(function() { stageGallerySlideTn(dir); }, 60);
			},function(){
				clearInterval(window.stageGalleryInterval);
			});
			window.stageGallerySlideTn = function(dir) {
				var targ = $('.stage-gallery .stage-gallery-tn');
				var l = parseInt(targ.css('left').replace(/px/,''));
				var min_x = parseInt(targ.data('min_x'));
				if (dir=='left') {
					
					if (l>min_x+8) {
						
						targ.css('left',(l-8)+'px');
					} else {
						targ.css('left',(min_x)+'px');
					}
				} else {
					if (l<4) {
						targ.css('left',(l+8)+'px');
